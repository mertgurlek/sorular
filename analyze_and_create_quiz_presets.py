...
# YDS Gerçek Dağılım (80 soru için)
YDS_FULL_DISTRIBUTION = {
    "YDS Kelime Soruları": 10,
    "YDS Gramer": 10,
    "YDS Cümle Tamamlama": 10,
    "YDS Çeviri Soruları": 12,  # 6 İng-Tr + 6 Tr-İng
    "YDS Diyalog": 5,
...
# Alternatif kategori isimleri (database'de farklı isimler olabilir)
CATEGORY_ALIASES = {
    ...
    "YDS Eş Anlam": ["YDS Eş Anlam", "Eş Anlam", "YDS Durum"],  # Durum da ekledik
...
def analyze_database():
    ...
    # Tüm kategorileri ve soru sayılarını al
    cur.execute("""
    ...
    # Doğru cevabı olmayan soruları kontrol et
    cur.execute("""
        SELECT category, COUNT(*) as count
        FROM questions
...
def generate_quiz_presets_json():
    ...
    for size, name in [('mini', 'Mini Quiz (20)'), ('medium', 'Orta Quiz (40)'), ('full', 'Tam YDS (80)')]:
        ...
        # Sadece question ID'lerini kaydet (daha kompakt)
        presets[size] = {
            'name': name,
...
def generate_js_distribution():
    """app.js için YDS dağılım fonksiyonunu oluştur"""
...
'''
    ...
    print("\n" + "="*60)
    print("📜 JavaScript Dağılım Kodu")
    print("="*60)
    print(js_code)
    
    # JS dosyasına kaydet
    with open('yds_distribution.js', 'w', encoding='utf-8') as f:
        f.write(js_code)
    
    print(f"\n✅ JS kodu kaydedildi: yds_distribution.js")
...
def main():
    ...
    # 4. JavaScript kodu oluştur
    generate_js_distribution()
    ...
    print("2. yds_distribution.js içeriğini app.js'e ekleyin")
    print("3. startExam fonksiyonunu güncelleyin")
